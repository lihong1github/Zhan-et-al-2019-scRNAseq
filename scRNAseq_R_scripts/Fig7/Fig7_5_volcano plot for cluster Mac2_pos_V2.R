...
# df <- dplyr::left_join(df, mac2_genes_devstages, by="Gene")

# genes_to_plot <- rbind(top_n(df[df$Cluster=="YS",], n=5, Log2FC),
#                        top_n(df[df$Cluster=="E1",], n=5, Log2FC),
#                        top_n(df[df$Cluster=="A1",], n=5, -Log2FC),
#                        top_n(df[df$Cluster=="A2",], n=5, -Log2FC))
...
grep("Kit", rownames(experiment.aggregate@data),  value = T)
grep("Kit", df$Gene,  value = T)
...
ggplot() + 
  ...
  geom_text_repel(data=genes_to_plot,
                  ...
                  color="black", fontface = 'bold',size = 6, box.padding = 0.5,
                  point.padding = 0.5, segment.size=0.25, segment.colour="black") +
  ylab("-Log10[FDR]") + xlab("Log2FC[Mac2+Ctrl cells\n vs Homeostatic MG1]") +
  theme_bw()+
...
ggsave("Volcano_Mac2_pos_genes_ctrl.pdf", plot = last_plot(), device = "pdf", path = "~/Desktop/",
