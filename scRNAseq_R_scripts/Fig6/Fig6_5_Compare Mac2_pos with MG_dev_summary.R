...
df_mac2 <- read.csv(file="~/desktop/markers_UMI_cluster-Mac2_pos.csv", stringsAsFactors = F, header = T)
...
ggsave("Mac2_UP_devstages.pdf", plot = last_plot(), device = "pdf", path = "~/Desktop/",
...
ggsave("Mac2_DN_devstages.pdf", plot = last_plot(), device = "pdf", path = "~/Desktop/",
       scale = 0.8, width = 8, height = 4, units = c("in"),
       dpi = 600, limitsize = FALSE)
