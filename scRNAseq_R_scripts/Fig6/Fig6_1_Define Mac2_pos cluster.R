...
cell_index <- which(experiment.aggregate@meta.data$lgals3.data>lgals3.cutoff) 
cell_name <- rownames(experiment.aggregate@meta.data)[cell_index]

# add a level to ident 
levels(experiment.aggregate@ident) <- c(levels(experiment.aggregate@ident), "Mac2_pos")
experiment.aggregate@ident[names(experiment.aggregate@ident) %in% cell_name] <- c("Mac2_pos")
table(experiment.aggregate@ident)

save(experiment.aggregate, file="~/desktop/PLX_Rdata/Data_RunTSNE_add_Mac2_pos.Rdata")
